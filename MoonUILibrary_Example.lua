...
if success and result then
    ...
    if success2 and module then
        UILibrary = module
        print("[Init]: MoonUILibrary loaded successfully")
    else
        error("[Init]: Failed to execute MoonUILibrary: " .. tostring(module))
    end
else
    warn("[Init]: HttpGet failed, attempting local load")
    local success3, localModule = pcall(function()
        return require(game.StarterPlayerScripts.MoonUILibrary)
    end)
    if success3 and localModule then
        UILibrary = localModule
        print("[Init]: Local MoonUILibrary loaded successfully")
    else
        error("[Init]: Failed to load MoonUILibrary locally: " .. tostring(localModule))
    end
end
